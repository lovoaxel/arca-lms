...
## 2026-03-08 — Ideas del día

- **Simulacro de Examen con IA:** Antes de cada parcial, un botón "Preparar examen" dentro de la materia analiza todos los materiales subidos por el profesor (PDFs, slides, lecturas) y genera un cuestionario de práctica cronometrado con 10-15 preguntas de opción múltiple y respuesta corta. Al terminar, muestra un breakdown por tema: "Tema: Valuación de empresas — 60% correcto — necesitas repasar". Implementación: text extraction de PDFs (pdf-parse) + prompt engineering al LLM para generar preguntas con distractores plausibles + UI de examen con timer + scoring breakdown por unidad temática. Por qué: los flashcards ya se propusieron para repaso diario, pero esto es distinto — simula la presión del examen real con retroalimentación diagnóstica precisa, y ningún LMS universitario lo ofrece nativo.

- **Salas de Co-Estudio en Tiempo Real:** Dentro de cada materia, un panel lateral muestra quién más está conectado ahora ("3 compañeros estudiando Finanzas"). El alumno puede abrir una "sala" con presencia en tiempo real: ven los cursores activos de otros, pueden pegar notas colaborativas en un editor compartido (Yjs CRDT) y hacer preguntas rápidas sin salir del portal. El profesor puede unirse como "invitado silencioso" o activar una sesión de Q&A de 10 min antes del examen. Implementación: WebSockets para presencia (Socket.io) + editor colaborativo ligero con Yjs + lista de participantes activos por materia. Por qué: los estudiantes ya estudian juntos por WhatsApp y Meet, pero sin contexto del curso — traer esa colaboración al mismo lugar donde están los materiales reduce el cambio de contexto y construye comunidad dentro del portal.

- **Smart Syllabus Parser — Tu plan del semestre en 30 segundos:** Al inicio de cada materia, el alumno sube el temario PDF del profesor (o lo pega como texto). El sistema lo parsea con un LLM, extrae automáticamente: lista de temas con fechas estimadas, bibliografía obligatoria con búsqueda automática en la biblioteca digital de Anáhuac, y genera un plan de estudio semanal distribuido según la carga total del alumno en ese semestre. El resultado es arrastrable/editable y se sincroniza con el módulo de calendario. Implementación: PDF parsing + prompt estructurado para extraer entidades (tema, fecha, bibliografía) + algoritmo de distribución de carga entre materias + UI drag-and-drop. Por qué: el 80% de los alumnos nunca leen el syllabus completo porque es un PDF aburrido — transformarlo en un mapa de vuelo interactivo convierte la planificación pasiva en un hábito activo desde el día 1.
